...
pub(crate) fn generate(table: &mut [Operation]) {
    generate_lsd_reg(table);
    generate_lsd_mem(table);
}

fn generate_lsd_reg(table: &mut [Operation]) {
    let base_mask = 0b1110000000001000;
                ...
                for mode in 0..=1 {
                    for reg in 0..=0b111 {
                        ...
                        let rotation_info = match mode {
                            ...
                            _ => panic!("generated_lsd_reg: unexpected mode bit {}", mode),
                        };
...
fn generate_lsd_mem(table: &mut [Operation]) {
    let base_mask = 0b1110001011000000;
