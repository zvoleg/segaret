...
#[derive(Copy, Clone)]
pub(in crate) enum RegisterType {
    Address,
    Data,
}

#[derive(Copy, Clone)]
pub(in crate) struct Register {
    pub(in crate) reg_type: RegisterType,
...
#[derive(Copy, Clone)]
pub(in crate) struct Location {
    location_type: LocationType,
    address: usize,
}

impl Location {
    pub(in crate) fn new(location_type: LocationType, address: usize) -> Self {
        Self {
            location_type,
            address,
        }
    ...
    pub(in crate) fn memory(address: usize) -> Self {
        Self::new(LocationType::Memory, address)
    }

    pub(in crate) fn register(register: Register) -> Self {
...
impl Register {
    pub(in crate) fn new(reg_type: RegisterType, reg_idx: usize) -> Self {
        Register { reg_type, reg_idx }
    }

    pub(in crate) fn data(reg_idx: usize) -> Self {
        Register::new(RegisterType::Data, reg_idx)
    }

    pub(in crate) fn addr(reg_idx: usize) -> Self {
