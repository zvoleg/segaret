use hardware::{ sign_extend, Size };
use std::fmt;

use crate::mc68k_emu::Mc68k;

use crate::{RegisterType, Register, Location};
...
impl AddrModeType {
    pub(in crate) fn get_clock_periods_short(&self) -> u32 {
        match self {
            AddrModeType::Data => 0,
            AddrModeType::Addr => 0,
    ...
    pub(in crate) fn get_clock_periods_long(&self) -> u32 {
        match self {
            AddrModeType::Data => 0,
            AddrModeType::Addr => 0,
...
#[derive(Copy, Clone)]
pub(in crate) struct BriefExtWord {
    pub(in crate) register: Register,
    pub size: Size,
...
#[derive(Copy, Clone)]
pub(in crate) struct AddrMode {
    pub(in crate) am_type: AddrModeType, 
    pub(in crate) reg_idx: usize,
    pub(in crate) ext_word: Option<u32>,
    pub(in crate) brief_ext_word: Option<BriefExtWord>,
    pub(in crate) ext_word_addr: u32,
}

impl AddrMode {
    pub(in crate) fn new(am_type: AddrModeType, reg_idx: usize) -> Self {
        Self {
            am_type: am_type,
            reg_idx: reg_idx,
            ext_word: None,
    ...
    pub(in crate) fn fetch_ext_word(&mut self, cpu: &mut Mc68k, size: Size) {
        match self.am_type {
